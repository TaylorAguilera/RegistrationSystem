...
if ($resC->num_rows > 0) {
echo $row["Group_by"] .":".$row["total"]."/10 users. <br />";
}
}
...
WHERE `Email`='$email'";
$sqlG = "SELECT * FROM `users` WHERE Group_by='$group_list1'";
$resG = mysqli_query($link,$sqlG);
while($row = $resG->fetch_assoc()){
  if($resG->num_rows >= 10){
  echo 'Sorry, This class has reached the user limit Please try again later, or contact the admin at (WOU@secretary.edu) with your email provided by the university and group number.';
  exit();
